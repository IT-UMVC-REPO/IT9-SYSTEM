...
test('video call client uses native rtc peer connection and server-provided ice configuration', function () {
    ...
    expect($app)
        ...
        ->toContain('window.sukiRingtone?.start();')
        ->toContain('window.sukiRingtone?.stop();')
        ...
        ->toContain('hasMicrophone')
        ->toContain('hasCamera')
        ->toContain('this.microphoneMuted = true')
        ->toContain('this.cameraDisabled = true')
        ...
        ->toContain('document.pictureInPictureEnabled')
        ...
        ->toContain('endingCall: false')
        ->toContain('if (this.endingCall || this.callStatus === \'idle\' || this.callStatus === \'ended\')')
        ->toContain('this.endingCall = true')
        ->toContain('if (this.callStatus === \'ended\' && nextStatus === \'ended\')')
        ->toContain('remoteVideoActive')
        ->toContain('setMaxBitrate')
        ->toContain('window.__activeGroupCall')
        ->toContain('const videoMaxKbps = isMobile ? 900 : 1800;')
        ->toContain('participantFullscreenId')
        ->toContain('handleParticipantTap')
        ->toContain('toggleParticipantFullscreen')
        ->toContain('participantFullscreenClass')
        ->toContain('document.pictureInPictureEnabled')
        ->toContain('requestPictureInPicture')
        ->toContain('enterPip')
...
test('conversation keeps video call alpine controls stable during livewire refreshes', function () {
    ...
    expect($conversation)
        ...
        ->toContain("screenSharing ? 'opacity-100' : (cameraDisabled ? 'opacity-0' : 'opacity-100 scale-x-[-1]')")
        ...
        ->toContain('x-bind:disabled="! hasMicrophone"')
        ->toContain('x-bind:disabled="! hasCamera || screenSharing"')
...
test('native pip returns without persistent call overlay state', function () {
    $app = file_get_contents(resource_path('js/app.js'));
    $conversation = messagingBladeSource('conversation');
...
test('group conversation call overlay uses desktop tiles and a mobile filmstrip', function () {
    ...
    expect($groupConversation)
        ...
        ->toContain("activeCallInProgress('[data-group-video-call]')")
        ...
        ->toContain('handleParticipantTap(participant.id)')
        ...
        ->toContain('handleParticipantTap(participant.id)')
        ->toContain('participantFullscreenClass(participant.id)')
        ->toContain('remoteParticipants.length === 0')
        ->toContain('h-full min-h-0 flex-col')
