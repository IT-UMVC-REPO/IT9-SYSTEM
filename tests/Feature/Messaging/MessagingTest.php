...
test('messaging views use realtime-first fallback polling', function () {
    ...
    expect($conversation)
        ...
        ->toContain('shouldUseFallbackPolling()')
        ->toContain("return state !== null && state !== 'connected';")
        ->toContain('pollInFlight: false')
        ->toContain('document.hidden')
        ->toContain('pendingDirectMessages')
        ->toContain('direct-message-pending')
        ->toContain('x-model="draftMessage"')
        ->toContain('showEmoji: false')
        ->toContain('$wire.send(content)')
        ->not->toContain('relative" x-data="{ showEmoji: false }"')
        ->toContain('}, 30000);')
        ...
        ->toContain("return state !== null && state !== 'connected';")
        ->toContain('pollInFlight: false')
        ->toContain('pendingGroupMessages')
        ->toContain('group-message-pending')
        ->toContain('x-model="draftMessage"')
        ->toContain('showEmoji: false')
        ->toContain('$wire.send(content)')
        ->not->toContain('relative" x-data="{ showEmoji: false }"')
