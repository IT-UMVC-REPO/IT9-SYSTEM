...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    @php
        ...
        if ($user === null) {
            ...
            $navigationItems = [
                ['label' => __('Features'), 'route' => '#features', 'patterns' => [], 'icon' => 'fa-solid fa-sparkles'],
                ['label' => __('How it works'), 'route' => '#how-it-works', 'patterns' => [], 'icon' => 'fa-solid fa-seedling'],
                ['label' => __('Portals'), 'route' => '#portals', 'patterns' => [], 'icon' => 'fa-solid fa-store'],
            ];
            $quickActionItems = [];
            $portalSummary = __('Browse the storefront, discover trusted vendors, and create an account when you are ready to shop.');
            ...
            $logoRoute = $navigationItems[0]['route'];
        ...
        <header class="sticky top-0 z-50 border-b border-[#e7e5e4] bg-white/95 backdrop-blur-md">
            <nav class="mx-auto flex h-[52px] max-w-[1500px] items-stretch gap-4 px-4 sm:px-6 lg:px-8">
                <div class="flex shrink-0 items-center">
                    @auth
                        <x-app-logo href="{{ $logoRoute }}" wire:navigate class="shrink-0" />
                    @else
                        <x-app-logo href="{{ $logoRoute }}" class="shrink-0" />
                    @endauth
                </div>
                ...
                <div class="hidden flex-1 items-stretch justify-center lg:flex">
                    @foreach ($navigationItems as $item)
                        @php
                            $itemIsActive = filled($item['patterns']) && request()->routeIs(...$item['patterns']);
                            $isHashLink = str_starts_with($item['route'], '#');
                        @endphp

                        <a
                            href="{{ $item['route'] }}"
                            @if (! $isHashLink) wire:navigate @endif
                            class="flex h-full items-center gap-2 border-b-2 px-5 text-sm transition-colors {{ $itemIsActive ? 'border-emerald-600 font-semibold text-emerald-700' : 'border-transparent text-stone-500 hover:text-stone-800' }}"
                        >
                        ...
                        <div class="border-b border-stone-200 bg-stone-50 px-5 py-4">
                            ...
                            @else
                                <div class="rounded-[1.25rem] border border-stone-200 bg-white p-4">
                                    <p class="text-sm font-semibold text-neutral-900">Welcome to SukiMarket</p>
                                    <p class="mt-2 text-sm leading-6 text-neutral-600">
                                        Explore the digital palengke, then sign in when you are ready to save favorites and place orders.
                                    </p>
                                </div>
                            @endauth
                        ...
                        <div class="grid gap-2 p-4">
                            @foreach ($mobileNavigationItems as $item)
                                ...
                                <a
                                    href="{{ $item['route'] }}"
                                    @if (! $isHashLink) wire:navigate @endif
                                    class="flex items-center justify-between gap-3 rounded-[1.25rem] border px-4 py-3 text-sm font-semibold transition {{ $itemIsActive ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-stone-200 text-neutral-700 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-700' }}"
                                >
