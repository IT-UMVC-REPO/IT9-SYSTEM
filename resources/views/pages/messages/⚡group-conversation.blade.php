                                                    ...
                                                    @foreach ($attachments as $attachment)
                                                        ...
                                                        @if (\Illuminate\Support\Str::startsWith($attachmentMime, 'image/'))
                                                            <a href="{{ $attachmentUrl }}" target="_blank" rel="noopener noreferrer" class="block overflow-hidden rounded-2xl border {{ $isOwnMessage ? 'border-white/25' : 'border-neutral-200 dark:border-neutral-700' }}">
                                                                <img src="{{ $attachmentUrl }}" alt="{{ __('Attached image') }}" referrerpolicy="no-referrer" onerror="this.onerror=null; this.classList.add('hidden'); this.nextElementSibling.classList.remove('hidden'); this.nextElementSibling.classList.add('flex');" class="max-h-52 w-full object-cover" loading="lazy">
                                                                <span class="hidden min-h-24 w-full items-center justify-center bg-neutral-100 px-4 py-6 text-center text-xs font-medium text-neutral-500 dark:bg-neutral-800 dark:text-neutral-300">{{ __('Image unavailable') }}</span>
                                                            </a>
                                                        @else
