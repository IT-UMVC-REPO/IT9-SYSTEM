...
new #[Title('Create product')] class extends Component {
    ...
    {
        return Category::query()->leaves()->with('parent:id,name')->orderBy('name')->get()->groupBy(fn(Category $category) => $category->parent?->name ?? __('Other'));
    }
    ...
    public function selectedCategory(): ?Category
    {
        ...
        return Category::query()->with('parent:id,name')->find((int) $this->categoryId);
    }
    ...
    public function selectedCategoryBreadcrumb(): ?string
    {
        ...
        return $this->selectedCategory->parent ? $this->selectedCategory->parent->name . ' / ' . $this->selectedCategory->name : $this->selectedCategory->name;
    }
    ...
    {
        return array_values(array_filter(ProductUnit::cases(), fn(ProductUnit $unit): bool => !in_array($unit, $this->suggestedUnitOptions, true)));
    }
    ...
    public function conversionUnitOptions(): array
    {
        ...
        return array_map(
            ...
            array_values(array_filter(ProductUnit::cases(), fn(ProductUnit $unit): bool => $unit !== $this->selectedUnit && ($this->selectedUnit->isCountBased() ? true : $this->selectedUnit->type() === $unit->type()))),
        );
    }
    ...
    public function unitFormulaPreview(): ?string
    {
        ...
        $price = is_numeric($this->price) ? UnitFormatter::currency((float) $this->price) : UnitFormatter::currency(0);
    ...
    public function unitConversionResult(): ?UnitConversionResult
    {
        ...
        if (!$this->showUnitConversion || $this->selectedUnit === null || $conversionUnit === null || blank($this->conversion_unit_quantity) || !is_numeric($this->conversion_unit_quantity)) {
            return null;
        }
        ...
        return new UnitConversionResult(fromUnit: $this->selectedUnit, fromQuantity: 1, toUnit: $conversionUnit, toQuantity: (float) $this->conversion_unit_quantity, fromUnitPrice: is_numeric($this->price) ? (float) $this->price : null);
    }
    ...
    private function canonicalStockAttributes(array $defaultVariant, array $variantRows): array
    {
        ...
        $canUseCanonical = $defaultUnit->conversionFactor() !== null && collect($variantRows)->every(fn(array $row): bool => ProductUnit::from($row['unit'])->isConvertibleTo($defaultUnit));
...
<div class="mx-auto max-w-[1500px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
    <form wire:submit="save" class="space-y-6" x-data="{
        ...
        <header
            class="flex flex-col gap-4 border-b border-stone-200 pb-5 dark:border-white/10 lg:flex-row lg:items-end lg:justify-between">
            <div class="min-w-0">
                <a href="{{ route('vendor.products') }}" wire:navigate
                    class="brand-hover-text inline-flex items-center gap-2 text-sm font-semibold text-neutral-500 dark:text-zinc-400">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                <a href="{{ route('vendor.products') }}" wire:navigate
                    class="brand-button-secondary active:scale-[0.96] justify-center">
                    {{ __('Cancel') }}
                </a>
                <flux:button variant="primary" type="submit" wire:loading.attr="disabled"
                    wire:target="save,productImageUpload" class="justify-center">
            ...
            <main class="space-y-6">
                <section class="brand-panel overflow-hidden p-0">
                    <div class="grid lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
                        <div class="relative min-h-[24rem] bg-stone-100 dark:bg-zinc-900"
                            ...
                            <input x-ref="productImageInput" id="product-image-upload" type="file"
                                wire:model="productImageUpload" accept="image/jpeg,image/png,image/webp"
                                class="sr-only">

                            @if ($productImageUpload instanceof \Livewire\Features\SupportFileUploads\TemporaryUploadedFile)
                                <img src="{{ $productImageUpload->temporaryUrl() }}" alt="{{ __('Product preview') }}"
                                    class="h-full min-h-[24rem] w-full object-cover" loading="lazy">

                                <div
                                    class="absolute inset-x-4 bottom-4 flex flex-wrap items-center justify-between gap-3">
                                    <label for="product-image-upload"
                                        class="cursor-pointer rounded-lg bg-neutral-950/75 px-3.5 py-2 text-sm font-semibold text-white shadow-sm backdrop-blur transition hover:bg-neutral-950/90">
                                        {{ __('Change photo') }}
                                    </label>
                            ...
                            @else
                                <label for="product-image-upload"
                                    ...
                                    <span>
                                        <span
                                            class="block text-lg font-bold text-neutral-950 dark:text-zinc-100">{{ __('Add a product photo') }}</span>
                                        <span
                                            class="mt-2 block max-w-sm text-sm leading-6 text-neutral-500 dark:text-zinc-400">
                            ...
                            <div wire:loading.flex wire:target="productImageUpload"
                                class="absolute inset-0 hidden flex-col items-center justify-center gap-3 bg-white/80 backdrop-blur dark:bg-zinc-950/75">
                                <span
                                    class="h-11 w-11 animate-spin rounded-full border-4 border-[var(--brand-200)] border-t-[var(--brand-600)]"></span>
                                <span
                                    class="text-sm font-semibold text-neutral-700 dark:text-zinc-200">{{ __('Uploading...') }}</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-5 p-5 sm:p-6">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <h2 class="text-xl font-bold text-neutral-950 dark:text-zinc-100">
                                        {{ __('Name and story') }}</h2>
                                </div>
                            </div>

                            <div class="grid gap-4">
                                <flux:input wire:model.live.debounce.250ms="name" :label="__('Product name')"
                                    :placeholder="__('e.g. Sariwang Bangus - Davao Gulf')" maxlength="255" required
                                    class:input="h-11" />
                                <div class="-mt-2 text-right text-xs text-neutral-400 dark:text-zinc-500">
                                    {{ mb_strlen($name) }}/255</div>
                                <flux:error name="name" />

                                <flux:textarea wire:model.live.debounce.250ms="description" :label="__('Description')"
                                ...
                                <div class="-mt-2 text-right text-xs text-neutral-400 dark:text-zinc-500">
                                    {{ mb_strlen($description) }}/1000</div>
                    ...
                    <div class="grid gap-6 xl:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)] xl:items-start">
                        <div class="space-y-5">
                            ...
                            <flux:select wire:model.live="categoryId" :label="__('Category')"
                                ...
                                @foreach ($this->categoryGroups as $parentName => $categories)
                                    <optgroup label="{{ $parentName }}"
                                        wire:key="category-group-{{ str($parentName)->slug() }}">
                            ...
                            @if ($this->selectedCategoryBreadcrumb)
                                <span
                                    class="inline-flex w-fit rounded-lg border border-stone-200 bg-stone-50 px-3 py-1.5 text-xs font-semibold text-neutral-600 dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300">
                        ...
                        <div class="space-y-5">
                            ...
                            <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(11rem,0.85fr)]">
                                <flux:field>
                                    ...
                                    <flux:input.group>
                                        ...
                                        <flux:input wire:model.live.debounce.250ms="price" type="number"
                                            inputmode="decimal" step="0.01" min="0.01" class:input="h-11"
                                            required />
                                    </flux:input.group>
                            ...
                            <flux:callout icon="information-circle" variant="secondary" class="max-w-2xl">
                                <flux:callout.text>
                                    {{ __('Price is per selected selling unit. Stock is the number of those units available.') }}
                                </flux:callout.text>
                            </flux:callout>
                        </div>
                    </div>
                ...
                <section class="brand-panel p-5 sm:p-6" wire:key="unit-selector-{{ $categoryId }}">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h2 class="text-xl font-bold text-neutral-950 dark:text-zinc-100">
                                    {{ __('How this item is sold') }}</h2>
                            </div>
                        </div>

                        @if ($this->unitFormulaPreview)
                            <p
                                class="rounded-lg border border-[var(--brand-200)] bg-[var(--brand-50)] px-4 py-3 text-sm font-semibold text-[var(--brand-800)] dark:border-[var(--brand-500)]/20 dark:bg-[var(--brand-500)]/10 dark:text-[var(--brand-200)]">
                    ...
                    <div class="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                        @foreach ($this->suggestedUnitOptions as $unitOption)
                            <button type="button" wire:key="suggested-unit-{{ $unitOption->value }}"
                                ...
                                @class([
                                    ...
                                    'border-2 border-[var(--brand-500)] bg-[var(--brand-600)] text-white shadow-sm' =>
                                        $unit === $unitOption->value,
                                    'border-stone-200 bg-stone-50 text-neutral-700 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300' =>
                                        $unit !== $unitOption->value,
                                ])>
                    ...
                    <details
                        class="mt-4 rounded-lg border border-stone-200 bg-stone-50 p-4 dark:border-white/10 dark:bg-zinc-900">
                        ...
                        <div class="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-3 xl:grid-cols-4">
                            @foreach ($this->otherUnitOptions as $unitOption)
                                <button type="button" wire:key="other-unit-{{ $unitOption->value }}"
                                    wire:click="$set('unit', '{{ $unitOption->value }}')"
                                    @class([
                                        ...
                                        'border-2 border-[var(--brand-500)] bg-[var(--brand-600)] text-white' =>
                                            $unit === $unitOption->value,
                                        'border-stone-200 bg-white text-neutral-700 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-300' =>
                                            $unit !== $unitOption->value,
                                    ])>
                    ...
                    <div class="mt-5 border-t border-stone-200 pt-5 dark:border-white/10">
                        <label
                            class="flex cursor-pointer items-center justify-between gap-4 rounded-lg border border-dashed border-stone-300 bg-stone-50 p-4 dark:border-zinc-700 dark:bg-zinc-900">
                            <span class="flex min-w-0 items-start gap-3">
                                <span
                                    class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-white text-[var(--brand-600)] shadow-sm dark:bg-zinc-950">
                                    <i class="fa-solid fa-arrows-left-right"></i>
                                </span>
                                <span>
                                    <span
                                        class="block text-sm font-bold text-neutral-900 dark:text-zinc-100">{{ __('Unit conversion') }}</span>
                                    <span
                                        class="mt-1 block text-xs leading-5 text-neutral-500 dark:text-zinc-400">{{ __('Optional - declare the base quantity per unit') }}</span>
                                </span>
                            </span>
                            <flux:checkbox wire:model.live="showUnitConversion" />
                        </label>

                        <div x-cloak x-show="showConversion" x-transition:enter="transition ease-out duration-300"
                            x-transition:enter-start="opacity-0 -translate-y-3 scale-98"
                            x-transition:enter-end="opacity-100 translate-y-0 scale-100"
                            x-transition:leave="transition ease-in duration-200"
                            x-transition:leave-start="opacity-100 translate-y-0 scale-100"
                            x-transition:leave-end="opacity-0 -translate-y-3 scale-98" class="mt-5 space-y-4">
                            ...
                            @if ($this->conversionUnitOptions === [])
                                <flux:callout icon="information-circle" variant="secondary">
                                    <flux:callout.text>
                                        {{ __("Conversion is not needed - you're already selling by a conversion unit.") }}
                                    </flux:callout.text>
                                </flux:callout>
                            @else
                                <div class="grid gap-5 lg:grid-cols-[minmax(0,0.85fr)_minmax(0,1.15fr)]">
                                                ...
                                                @foreach ($this->conversionUnitOptions as $baseUnitOption)
                                                    <button type="button"
                                                        ...
                                                        @class([
                                                            ...
                                                            'border-2 border-[var(--brand-500)] bg-[var(--brand-600)] text-white' =>
                                                                $conversion_unit === $baseUnitOption['value'],
                                                            'border-stone-200 bg-white text-neutral-600 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300' =>
                                                                $conversion_unit !== $baseUnitOption['value'],
                                                        ])>
                                        ...
                                        <flux:field>
                                            ...
                                            <div class="relative">
                                                <flux:input type="number"
                                                    wire:model.live.debounce.250ms="conversion_unit_quantity"
                                                    step="0.001" min="0.001" max="99999"
                                                    x-on:input="if (parseFloat($el.value) > 99999) { $el.value = 99999; $wire.$set('conversion_unit_quantity', '99999'); }"
                                                    :placeholder="__('e.g. 25')" class="pr-16" />
                                                @if (filled($conversion_unit))
                                                    <span
                                                        class="pointer-events-none absolute right-2 top-1/2 -translate-y-1/2 rounded-lg bg-stone-100 px-2.5 py-1 text-xs font-bold text-neutral-500 dark:bg-zinc-800 dark:text-zinc-300">{{ $conversion_unit }}</span>
                                                @endif
                                            </div>
                                    ...
                                    <div
                                        class="rounded-lg border border-[var(--brand-200)] bg-[var(--brand-50)] p-5 dark:border-[var(--brand-500)]/20 dark:bg-[var(--brand-500)]/10">
                                        ...
                                        <div class="mt-4 space-y-2">
                                            @if ($this->unitConversionResult)
                                                <p class="text-2xl font-bold text-neutral-900 dark:text-zinc-100">
                                                    {{ $this->unitConversionResult->displayString }}</p>
                                                @if ($this->unitConversionResult->pricePerConversionUnitLabel())
                                                    <p
                                                        class="text-sm font-semibold text-neutral-600 dark:text-zinc-300">
                                                        {{ $this->pricePreview }} <span
                                                            aria-hidden="true">&rarr;</span>
                                                        {{ $this->unitConversionResult->pricePerConversionUnitLabel() }}
                                                    </p>
                                                @endif
                ...
                <section class="brand-panel p-5 sm:p-6">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <div>
                                ...
                                <h2 class="text-xl font-bold text-neutral-950 dark:text-zinc-100">
                                    {{ __('Offer more than one unit') }}</h2>
                            </div>
                        </div>
                    ...
                    <div class="mt-5 grid gap-4">
                        <div @class([
                            ...
                            'border-[var(--brand-500)] bg-[var(--brand-50)] dark:border-[var(--brand-500)]/40 dark:bg-[var(--brand-500)]/10' =>
                                $defaultVariantKey === 'primary',
                            'border-stone-200 bg-stone-50 dark:border-white/10 dark:bg-zinc-900' =>
                                $defaultVariantKey !== 'primary',
                        ])>
                            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                <div>
                                    <p class="text-sm font-bold text-neutral-900 dark:text-zinc-100">
                                        {{ __('Primary variant') }}</p>
                                    <p class="mt-1 text-sm text-neutral-500 dark:text-zinc-400">
                                        {{ $this->pricePreview ?? __('Set a price') }} <span
                                            aria-hidden="true">&middot;</span>
                                        {{ $selectedUnit ? __($selectedUnit->label()) : __('Selling unit') }} <span
                                            aria-hidden="true">&middot;</span>
                                        {{ __(':stock in stock', ['stock' => $stock_quantity ?: 0]) }}
                                    </p>
                                    @if ($this->unitConversionResult)
                                        <p
                                            class="mt-2 text-xs font-semibold text-[var(--brand-700)] dark:text-[var(--brand-300)]">
                                            {{ $this->unitConversionResult->displayString }}</p>
                                    @endif
                                </div>
                                <button type="button" wire:click="setDefaultVariant('primary')"
                                    class="inline-flex items-center justify-center rounded-full border px-3 py-1.5 text-xs font-bold transition {{ $defaultVariantKey === 'primary' ? 'border-[var(--brand-600)] bg-[var(--brand-600)] text-white' : 'border-stone-200 bg-white text-neutral-600 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-950 dark:text-zinc-300' }}">
                        ...
                        @foreach ($additionalVariants as $index => $variant)
                            ...
                            <div wire:key="additional-variant-{{ $index }}"
                                class="rounded-2xl border border-stone-200 bg-white p-4 dark:border-white/10 dark:bg-zinc-950">
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        <p class="text-sm font-bold text-neutral-900 dark:text-zinc-100">
                                            {{ __('Variant :number', ['number' => $loop->iteration + 1]) }}</p>
                                        <p class="mt-1 text-xs text-neutral-500 dark:text-zinc-400">
                                            {{ __('Each variant has its own price, stock, and optional conversion note.') }}
                                        </p>
                                    </div>
                                    <div class="flex flex-wrap gap-2">
                                        <button type="button"
                                            wire:click="setDefaultVariant('variant-{{ $index }}')"
                                            class="inline-flex items-center justify-center rounded-full border px-3 py-1.5 text-xs font-bold transition {{ $defaultVariantKey === 'variant-' . $index ? 'border-[var(--brand-600)] bg-[var(--brand-600)] text-white' : 'border-stone-200 bg-white text-neutral-600 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300' }}">
                                            {{ $defaultVariantKey === 'variant-' . $index ? __('Default') : __('Make default') }}
                                        </button>
                                        <button type="button" wire:click="removeVariant({{ $index }})"
                                            wire:confirm="{{ __('Delete this variant?') }}"
                                            @disabled($defaultVariantKey === 'variant-' . $index)
                                            class="inline-flex items-center justify-center rounded-full border border-rose-200 px-3 py-1.5 text-xs font-bold text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-40 dark:border-rose-500/30 dark:text-rose-300 dark:hover:bg-rose-500/10">
                                            {{ __('Delete') }}
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="mt-4 grid gap-4 md:grid-cols-3 md:items-start">
                                    <flux:field>
                                        ...
                                        <flux:select wire:model.live="additionalVariants.{{ $index }}.unit">
                                            @foreach (ProductUnit::cases() as $variantUnitOption)
                                                <flux:select.option :value="$variantUnitOption->value"
                                                    :label="$variantUnitOption->label().' ('.$variantUnitOption->symbol().')'" />
                                            @endforeach
                                        </flux:select>
                                    ...
                                    <flux:field>
                                        ...
                                        <flux:input.group>
                                            <flux:input.group.prefix>&#8369;</flux:input.group.prefix>
                                            <flux:input
                                                wire:model.live.debounce.250ms="additionalVariants.{{ $index }}.price"
                                ...
                                <details
                                    class="mt-4 rounded-xl border border-stone-200 bg-stone-50 p-4 dark:border-white/10 dark:bg-zinc-900">
                                    <summary
                                        class="cursor-pointer text-sm font-semibold text-neutral-900 dark:text-zinc-100">
                                        {{ __('Optional conversion note') }}</summary>
                                    <div class="mt-4 grid gap-4 sm:grid-cols-2">
                                        <flux:field>
                                            <flux:label>{{ __('Conversion unit') }}</flux:label>
                                            <flux:select
                                                wire:model.live="additionalVariants.{{ $index }}.conversion_unit"
                                                placeholder="{{ __('None') }}">
                                                <flux:select.option value="" :label="__('None')" />
                                                @foreach (ProductUnit::cases() as $conversionUnitOption)
                                                    <flux:select.option :value="$conversionUnitOption->value"
                                                        :label="$conversionUnitOption->label().' ('.$conversionUnitOption->symbol().')'" />
                                                @endforeach
                                            </flux:select>
                                            <flux:error
                                                name="additionalVariants.{{ $index }}.conversion_unit" />
                                        </flux:field>

                                        <flux:field>
                                            <flux:label>{{ __('Quantity per variant') }}</flux:label>
                                            <flux:input
                                                wire:model.live.debounce.250ms="additionalVariants.{{ $index }}.conversion_unit_quantity"
                                                type="number" min="0.001" max="99999" step="0.001" />
                                            <flux:error
                                                name="additionalVariants.{{ $index }}.conversion_unit_quantity" />
                                        </flux:field>
                                    </div>
                                </details>
                            </div>
                ...
                <div class="xl:hidden">
                    <flux:button variant="primary" type="submit" wire:loading.attr="disabled"
                        wire:target="save,productImageUpload" class="w-full justify-center py-4 text-base">
                        <span wire:loading.remove wire:target="save">{{ __('Save product') }}</span>
                        <span wire:loading wire:target="save">{{ __('Saving product...') }}</span>
                    </flux:button>
                </div>
                ...
                <section class="brand-panel p-5">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <h2 class="text-xl font-bold text-neutral-950 dark:text-zinc-100">{{ __('Visibility') }}
                            </h2>
                        </div>
                    </div>

                    <div class="mt-5 grid gap-3">
                        <button type="button" wire:click="$set('status', '{{ ProductStatus::Inactive->value }}')"
                            ...
                            @class([
                                ...
                                'border-2 border-[var(--brand-500)] bg-[var(--brand-50)] text-[var(--brand-900)] dark:bg-[var(--brand-500)]/10 dark:text-[var(--brand-100)]' =>
                                    $status === ProductStatus::Inactive->value,
                                'border border-stone-200 bg-stone-50 text-neutral-700 hover:border-[var(--brand-300)] dark:border-white/10 dark:bg-zinc-900 dark:text-zinc-300' =>
                                    $status !== ProductStatus::Inactive->value,
                            ])>
                            @if ($status === ProductStatus::Inactive->value)
                                <span
                                    class="absolute right-3 top-3 flex h-5 w-5 items-center justify-center rounded-full bg-[var(--brand-600)] text-white">
                            ...
                            <p class="mt-1 text-xs leading-5 opacity-75">
                                {{ __('Hidden from storefront. Only you can see it.') }}</p>
                        </button>

                        <button type="button" wire:click="$set('status', '{{ ProductStatus::Active->value }}')"
                            aria-pressed="{{ $status === ProductStatus::Active->value ? 'true' : 'false' }}"
                            @class([
                                'relative rounded-lg p-4 text-left transition active:scale-[0.98]',
                                'border-2 border-[var(--brand-500)] bg-[var(--brand-600)] text-white' =>
                                    $status === ProductStatus::Active->value,
                                'border border-stone-200 bg-[var(--brand-50)] text-[var(--brand-800)] hover:border-[var(--brand-300)] dark:border-[var(--brand-500)]/20 dark:bg-[var(--brand-500)]/10 dark:text-[var(--brand-200)]' =>
                                    $status !== ProductStatus::Active->value,
                            ])>
                            @if ($status === ProductStatus::Active->value)
                                <span
                                    class="absolute right-3 top-3 flex h-5 w-5 items-center justify-center rounded-full bg-white text-[var(--brand-600)]">
                                    <i class="fa-solid fa-check text-[9px]"></i>
                                </span>
                            @endif
                            ...
                            <p class="mt-1 text-xs leading-5 opacity-75">
                                {{ __('Visible to shoppers on the storefront.') }}</p>
                        </button>
                    </div>
                ...
                <section class="brand-panel p-5">
                    ...
                    <div class="space-y-3 text-sm">
                        <div class="flex items-center justify-between gap-4">
                            ...
                            <span
                                class="truncate font-semibold text-neutral-900 dark:text-zinc-100">{{ $this->selectedCategoryBreadcrumb ?? __('Not selected') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <span class="text-neutral-500 dark:text-zinc-400">{{ __('Unit') }}</span>
                            <span
                                class="font-semibold text-neutral-900 dark:text-zinc-100">{{ $selectedUnit?->label() ?? __('Not selected') }}</span>
                        </div>
                        <div class="flex items-center justify-between gap-4">
                            <span class="text-neutral-500 dark:text-zinc-400">{{ __('Price') }}</span>
                            <span
                                class="font-semibold text-neutral-900 dark:text-zinc-100">{{ $this->pricePreview ?? __('Not priced') }}</span>
                        </div>
                    </div>
                </section>

                <flux:button variant="primary" type="submit" wire:loading.attr="disabled"
                    wire:target="save,productImageUpload" class="hidden w-full justify-center py-3 xl:flex">
                    <span wire:loading.remove wire:target="save">{{ __('Save product') }}</span>
                    <span wire:loading wire:target="save">{{ __('Saving product...') }}</span>
                </flux:button>
            </aside>
