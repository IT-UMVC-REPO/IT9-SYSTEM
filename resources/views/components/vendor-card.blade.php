...
<article {{ $attributes->merge(['class' => 'group relative flex h-full flex-col overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-black/5 transition-all duration-300 ease-out hover:-translate-y-1 hover:shadow-xl will-change-transform dark:bg-zinc-900 dark:ring-white/10']) }}>
    ...
    <div class="relative overflow-hidden">
        ...
        <h2 class="absolute bottom-4 left-4 right-4 font-serif text-lg font-bold leading-snug text-white drop-shadow-sm transition-opacity duration-200 line-clamp-2">
            {{ $vendor->store_name }}
        </h2>
        ...
        <div class="absolute left-3 top-3">
            <span class="inline-flex items-center gap-1 rounded-full bg-white/90 px-2.5 py-1 text-[11px] font-semibold text-neutral-700 shadow backdrop-blur-sm transition-all duration-200 group-hover:scale-105 group-hover:shadow-sm dark:bg-zinc-900/80 dark:text-zinc-200">
    ...
    <div class="flex flex-1 flex-col gap-3 p-4">
        ...
        {{-- Description --}}
        <p class="line-clamp-2 text-sm leading-relaxed text-neutral-500 transition-colors duration-200 dark:text-zinc-400">
